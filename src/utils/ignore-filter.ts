...
export class IgnoreFilter {
  ...
  processVulnerabilities(vulnerabilities: StandardVulnerability[]): ProcessedVulnerability[] {
  ...
  private findMatchingRule(vulnerability: StandardVulnerability): VulnIgnoreRule | undefined {
    return this.rules.find((rule) => {
      // CVE 必須符合
      if (rule.cve !== vulnerability.id) {
        return false;
      }

      // 如果規則指定了套件名稱，則須符合
      if (rule.package && rule.package !== vulnerability.packageName) {
        return false;
      }
